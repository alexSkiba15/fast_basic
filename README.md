...
- 🎯 Entry points to execute your program using `python -m <fast_basic>` or `$ fast_basic` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/alexSkiba15/fast_basic/branch/main/graph/badge.svg?token=fast_basic_token_here)](https://codecov.io/gh/alexSkiba15/fast_basic)
[![CI](https://github.com/alexSkiba15/fast_basic/actions/workflows/main.yml/badge.svg)](https://github.com/alexSkiba15/fast_basic/actions/workflows/main.yml)

Awesome fast_basic created by alexSkiba15

## Install

from source
```bash
git clone https://github.com/alexSkiba15/fast_basic fast_basic
cd fast_basic
make install
```
...
```bash
pip install fast_basic
```

## Executing

```bash
$ fast_basic run --port 8080
```

or

```bash
python -m fast_basic run --port 8080
```

or

```bash
$ uvicorn fast_basic:app
```
...
```bash
❯ fast_basic --help
Usage: fast_basic [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fast_basic create-user --help
Usage: fast_basic create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fast_basic create-user admin admin --superuser
```
...
Run with `fast_basic run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fast_basic create-user`.
...
-----------------------------------------------------
fast_basic/__init__.py              4      0   100%
fast_basic/app.py                  16      1    94%
fast_basic/cli.py                  21      0   100%
fast_basic/config.py                5      0   100%
fast_basic/db.py                   10      0   100%
fast_basic/models/__init__.py       0      0   100%
fast_basic/models/content.py       47      1    98%
fast_basic/routes/__init__.py      11      0   100%
fast_basic/routes/content.py       52     25    52%
fast_basic/routes/security.py      15      1    93%
fast_basic/routes/user.py          52     26    50%
fast_basic/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fast_basic/default.toml.
